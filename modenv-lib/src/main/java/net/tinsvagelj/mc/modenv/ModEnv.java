...
public enum ModEnv {
    ...
    ModEnv(String prettyName, String... detectClass) {
        this.prettyName = prettyName;
        this.detectClasses = detectClass;
    }

    public static ModEnv parse(String forceEnv) {
        String checked = forceEnv.toLowerCase().replace("_", "").replace("-", "");

        for (ModEnv it : ModEnv.values()) {
    ...
    public static Iterable<ModEnv> getAvailable() {
        java.util.ArrayList<ModEnv> result = new java.util.ArrayList<>(1);
        for (ModEnv env : ModEnv.values()) {
